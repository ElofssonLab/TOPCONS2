...
Options:
    ...
    -outformat              html|text : set the output format, default=$outformat
    -nc|--not-clean                   : do not clean the temporary dir

Modified from the original perl script by Nanjiang Shu, created 2010-08-27, updated 2010-09-13
...
if(@ARGV)#{{{
{
    ...
    while($ARGV[$i])
    {
        ...
        elsif($ARGV[$i] eq "-outformat" || $ARGV[$i] eq "--outformat" )
        {   
            $outformat = $ARGV[$i+1];
            $i +=2;
        }
...
# fixed 2010-09-13, no limitation
...
else #output text format
{
    unless (scalar(@seq)) {
        print STDERR  "No sequence entered!\n";
        exit;
    }

    if ($allow_sub) {
        print $fpout  "(Subsequence with lowest DG marked in orange Predicted DG\n";
    } 
    else 
    {
        print $fpout  "Sequence         \t Predicted_DG\n\n";
    }
    ...
    while($helix=shift(@seq)) {
        ...
        $helix =~ tr/a-z/A-Z/;
        unless ($helix =~ /^[ABCDEFGHIKLMNPQRSTUVWXYZ]+$/) {
            print $fpout  "$helix Sequence error.\n";
            next;
        }
        $Length=length($helix);
        if ($Length > 40) {
            print $fpout  "substr($helix,0,40) Length error (> 40 residues)\n";
            next;
        }
        if ($Length < 9) {
            print $fpout  "$helix Length error (< 9 residues)\n";
            next;
        }
        ...
        for (keys(%putative_TMs)) {
            ...
            if ($allow_sub) {
                ($ll,$ul)=split(/_/,$_);
                $helix = substr($helix,0,$ll-1) .  substr($helix,$ll-1,$ul-$ll+1) .  substr($helix,$ul);
            }
        }
        print $fpout  "$helix \t $helix_dG\n";
        $helix =~ s/^.*?\>//; $helix =~ s/<.*$//;
    }
    if ($fpout ne STDOUT)
    {
