...
if($#ARGV != 3) {
  printf "Usage: NN_pred2prf.pl <protnamefile> <predfile> <prfdir> <prfoutdir>\n";
  exit;
}
...
while(<PROTNAMEFILE>) {
	...
	or die "could not open prfoutfile ($outdir$seqname.prf) for $seqname";
    
    open(PRFFILE, "$prfdir"."$seqname".".prf")
	or die "could not open prffile ($prfdir"."$seqname".".prf) for $seqname";
    while(<PRFFILE>) {
	chomp;
	my $row = $_;
	...
	if($row =~ 'Sequence') {
	    ...
	    print PRFOUTFILE "ALPHABET:   M       -     <SPACE> <LABEL> <QUERY> \n";
	}
	elsif($row =~ 'COL') {
	    $row =~ s/\s+/ /g;
	    my @col = split / /, $row;
	    ...
	    printf PRFOUTFILE "%.2f    ", $preds[$predpos][0];
