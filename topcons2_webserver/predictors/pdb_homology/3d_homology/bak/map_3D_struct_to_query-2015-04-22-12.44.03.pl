...
#run JACKHMMER
system("jackhmmer -E 0.001 -A $alignment_file --tblout $hits_table --noali --acc -o /dev/null $query_file $DB_fasta");
...
my $query_id_aln = `tail -2 $alignment_AFA_oneline_file| head -1`;
	chomp $query_id_aln;
my $query_seq_aln = `tail -1 $alignment_AFA_oneline_file`;
	chomp $query_seq_aln;
