...
class Highway extends Module
{
    ...
    public Highway(String name, int distribType, int vertexType, int size, String label)
    {
	this.name = name;
	...
	for(ListIterator i = theVertices.listIterator();i.hasNext();) {
	    cur_nr = ((Vertex)i.next()).getNumber();
	    if(last_nr >= 0) {
		if(!addTransition(last_nr, cur_nr)) {
		    P.INTERNAL_ERROR("Highway.construct: Could not add intraconnections");
		}
	    }
	    last_nr = cur_nr;
	}
    ...
    public Highway(String name, double[] initDistrib, int vertexType, int size, String label)
    {
	this.name = name;
	this.vertexType = vertexType;
	...
	P.INTERNAL_ERROR("Highway.addTansition: vertex not in module");
	return false;
    }
