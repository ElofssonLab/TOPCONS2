...
#################### Workaround to keep only 2nd part hits from BLAST ##########################################
$blast_res = $ARGV[0];
$temp_blast_res = "$blast_res.TEMP";
open (INITIAL_BLAST, '<', $blast_res) || die "error opening $blast_res: $!";
$data_blast_res = do { local $/; <INITIAL_BLAST> };
close (INITIAL_BLAST);
@split_blast_res = split("done", $data_blast_res);

open OUT_TEMP, ">$temp_blast_res";
$split_blast_res[0]=~s/Searching\.+//g;
print OUT_TEMP $split_blast_res[0]."\n";
print OUT_TEMP "\n\n"."Searching..................................................done"."\n\n";
$split_blast_res[2]=~s/Results from round 2//;
print  OUT_TEMP $split_blast_res[2]."\n";
close OUT_TEMP;
#################### Workaround to keep only 2nd part hits from BLAST ##########################################

($file=$temp_blast_res) || die("Give blast output file name.");
open(IN,$file) || die("Couldn't open file $temp_blast_res");
